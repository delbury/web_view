...
const MATHCES = [
  {
    // 歌词时间轴
    key: 'timeAxis',
    reg: /^(?<times>\[\d{1,2}:\d{1,2}\.?\d*])+(?<lyric>.*)\s*$/,
    timeReg: /\[(.+?:.+?)\]/, // 时间正则
...
export default class LrcParser {
  ...
  parseLrcText(text) {
      ...
      for(let conf of MATHCES) {
        ...
        if(conf.reg.test(line)) {
          ...
          if(conf.key === 'timeAxis') {
            // 时间轴
            if(res.groups && res.groups.lyric) {
              const lrcObj = {};
              // 歌词原文、译文分割
            ...
            // 其他属性
            if(res.groups) {
              tempObj[conf.key] = conf.formatter ? conf.formatter(res.groups.content || '') : res.groups.content || '';
            }
          }
    ...
    console.log(count === 0 ? `歌词解析已完成` : `歌词解析未处理: ${count}`);
